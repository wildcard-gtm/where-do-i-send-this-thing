...
export default async function DashboardPage() {
  ...
  const teamUserIds = user ? await getTeamUserIds(user) : [];
  const userFilter = teamUserIds.length > 0 ? { userId: { in: teamUserIds } } : {};

  const [batches, contactCount, jobStats, postcardStats] = await Promise.all([
    user
      ...
      : [],
    user ? prisma.contact.count({ where: userFilter }) : 0,
    user
      ? prisma.job.aggregate({
          where: { batch: userFilter, status: "complete" },
          _count: true,
  ...
  const totalScans = user
    ? await prisma.batch.count({ where: userFilter })
    : 0;
  ...
  // Postcard summary
  const postcardTotal = (postcardStats as { status: string; _count: number }[]).reduce((s, r) => s + r._count, 0);
  const postcardReady = (postcardStats as { status: string; _count: number }[])
    .filter((r) => r.status === "ready" || r.status === "approved")
    .reduce((s, r) => s + r._count, 0);
  const postcardFailed = (postcardStats as { status: string; _count: number }[])
  ...
  const analyticsData: AnalyticsData = await (async () => {
    ...
    const [recCounts, allConfidences, jobStatusCounts, batchesForTrend, contactsForTrend] =
      await Promise.all([
        prisma.contact.groupBy({
          by: ["recommendation"],
          where: { ...userFilter, recommendation: { not: null } },
          _count: true,
        }),
        prisma.contact.findMany({
          where: { ...userFilter, confidence: { not: null } },
          select: { confidence: true },
        }),
        prisma.job.groupBy({
          by: ["status"],
          where: { batch: userFilter, status: { in: ["complete", "failed"] } },
          _count: true,
        }),
        prisma.batch.findMany({
          where: {
    ...
    // Contacts with no recommendation = "no result"
    const noResultCount = user
      ? await prisma.contact.count({ where: { ...userFilter, recommendation: null } })
      : 0;
    const totalContacts = contactCount;
    const noResultRate = totalContacts > 0 ? (noResultCount / totalContacts) * 100 : 0;
  ...
  const stats = [
    ...
    },
    {
      ...
      sub: jobStats._avg.confidence ? `${Math.round(jobStats._avg.confidence)}% avg confidence` : undefined,
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    },
    {
      ...
      value: postcardTotal,
      sub: postcardTotal > 0
        ? `${postcardReady} ready${postcardFailed > 0 ? ` · ${postcardFailed} failed` : ""}`
        : undefined,
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Here&apos;s an overview of your address lookups and postcards.
          </p>
        </div>
        <Link
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="text-foreground font-medium text-sm">
                        {batch.name || "Campaign"} &middot; {total} lead{total !== 1 ? "s" : ""}
                      </h3>
